...
type Draft = { id: string; cover: boolean; suggestedTitle?: string; suggestedAuthor?: string; sources: Source[] };
type Job = { id: string; status: string; phase?: string; progress: number; queuePosition?: number; title: string; destination?: 'output' | 'download'; outputName?: string; downloadReady?: boolean; error?: { code: string; message: string }; createdAt?: string };
type InputChapter = { filename: string; title: string };
type InputBook = { id: string; title: string; pathLabel: string; fileCount: number; suggestedTitle: string; suggestedAuthor: string; chapters: InputChapter[] };
...
function BatchReview({ items, setItems, busy, error, onBack, onQueue }: { items: BatchItem[]; setItems: (items: BatchItem[]) => void; busy: boolean; error: string; onBack: () => void; onQueue: () => void }) {
  ...
  return <section className="workspace-card psu-card batch-review" aria-labelledby="batch-review-title">
    <div className="section-heading"><div><p className="eyebrow">BATCH REVIEW</p><h1 id="batch-review-title">Review {items.length} audiobooks</h1><p>Check the compact book rows. Open a row only when its detailed metadata or chapter names need attention.</p></div><span className="step-pill">One at a time</span></div>
    ...
    <ol className="batch-list">
      {items.map((item, index) => <li key={item.book.id} className="batch-book">
        ...
        <div className="batch-book-fields">
          <label>Title<input value={item.metadata.title} required onChange={(event) => updateMetadata(index, { title: event.target.value })} /></label>
          <label>Author<input value={item.metadata.author} onChange={(event) => updateMetadata(index, { author: event.target.value })} /></label>
          <small>{item.book.pathLabel} · {item.chapters.length} chapter{item.chapters.length === 1 ? '' : 's'}</small>
          <details><summary>Edit details and chapter titles</summary><div className="batch-details form-stack two-columns">
            <label>Narrator<input value={item.metadata.narrator} onChange={(event) => updateMetadata(index, { narrator: event.target.value })} /></label>
            <label>Year<input inputMode="numeric" pattern="[0-9]{4}" value={item.metadata.year} onChange={(event) => updateMetadata(index, { year: event.target.value })} /></label>
            <label>Genre<input value={item.metadata.genre} onChange={(event) => updateMetadata(index, { genre: event.target.value })} /></label>
            ...
            <label className="full">Description<textarea rows={2} value={item.metadata.description} onChange={(event) => updateMetadata(index, { description: event.target.value })} /></label>
...
function Converter({ session, onJob, onBatchQueued }: { session: Session; onJob: (job: Job) => void; onBatchQueued: (jobs: Job[]) => void }) {
  ...
  async function loadInputBooks() { setInputLoading(true); setError(''); try { const result = await requestJson<{ mounted: boolean; books: InputBook[] }>('/api/input-books'); setInputMounted(result.mounted); setInputBooks(result.books); setSelected((current) => new Set([...current].filter((id) => result.books.some((book) => book.id === id)))); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The input folder could not be read.'); } finally { setInputLoading(false); } }
  ...
  function openDraft(next: Draft, suggestedTitle: string, suggestedAuthor = '') { setDraft(next); setMetadata({ ...defaultMetadata(), title: next.suggestedTitle ?? suggestedTitle, author: next.suggestedAuthor ?? suggestedAuthor }); }
  ...
  function startBatchReview() { const books = inputBooks.filter((book) => selected.has(book.id)); setBatchItems(books.map((book) => ({ book, chapters: book.chapters.map((chapter) => ({ ...chapter })), metadata: { ...defaultMetadata(), title: book.suggestedTitle, author: book.suggestedAuthor } }))); setError(''); }
  async function queueBatch() { if (!batchItems) return; setBusy(true); setError(''); try { const result = await requestJson<{ jobs: Job[] }>('/api/jobs/from-input/batch', { method: 'POST', headers: { 'x-csrf-token': session.csrf }, body: JSON.stringify({ items: batchItems.map((item) => ({ folderId: item.book.id, ...item.metadata, chapters: item.chapters })) }) }); onBatchQueued(result.jobs); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The batch could not be queued.'); } finally { setBusy(false); } }
  ...
  if (!draft) return <section className="source-layout" aria-labelledby="converter-title">
    ...
    <section className="workspace-card psu-card source-workspace">
      ...
      {sourceMode === 'input' ? inputLoading ? <div className="empty-state"><p>Scanning /input…</p></div> : !inputMounted ? <div className="empty-state"><img src="/vertiku-icon.png" alt="" /><h3>Input folder is not mounted</h3><p>Mount a host folder read-only at <code>/input</code>, then refresh.</p></div> : inputBooks.length === 0 ? <div className="empty-state"><img src="/vertiku-icon.png" alt="" /><h3>No audiobooks found</h3><p>Add one folder per audiobook. Audio files directly inside that folder become chapters.</p></div> : <>
        <div className="batch-toolbar"><div><strong>{selected.size} selected</strong><small>The persistent queue will process exactly one book at a time.</small></div><div className="button-row"><button className="text-button" onClick={() => setSelected(new Set(inputBooks.map((book) => book.id)))}>Select all</button><button className="text-button" disabled={selected.size === 0} onClick={() => setSelected(new Set())}>Clear</button><button className="primary" disabled={selected.size === 0 || busy} onClick={startBatchReview}>Batch review</button></div></div>
        <div className="input-book-list">{inputBooks.map((book) => <article key={book.id} className={`input-book ${selected.has(book.id) ? 'selected' : ''}`}><label className="book-select"><input type="checkbox" checked={selected.has(book.id)} onChange={() => toggleBook(book.id)} /><span className="visually-hidden">Select {book.title}</span></label><span className="folder-symbol" aria-hidden="true">▰</span><div><strong>{book.title}</strong><small>{book.pathLabel} · {book.fileCount} chapter{book.fileCount === 1 ? '' : 's'}</small></div><button className="secondary" disabled={busy} onClick={() => void analyzeInput(book)}>Review</button></article>)}</div>
...
function phaseLabel(job: Job) {
  ...
  if (job.status === 'running') return job.phase === 'preparing' ? 'Reading source audio' : job.phase === 'validating' ? 'Validating the finished M4B' : 'Encoding chapter audio';
...
function JobProgress({ job, session, onChange }: { job: Job; session: Session; onChange: (job: Job) => void }) {
  ...
  return <section className="workspace-card job-card" aria-live="polite"><JobActivity job={job} /><p className="eyebrow">CONVERSION JOB</p><h1>{job.title || 'Your audiobook'}</h1><p className="job-phase">{phaseLabel(job)}</p><p className="job-status">{job.status === 'completed' ? job.destination === 'output' ? `Saved to /output/${job.outputName ?? 'audiobook.m4b'}.` : 'Your validated browser download is ready.' : job.status === 'failed' ? 'The conversion could not be completed.' : job.status === 'cancelled' ? 'The conversion was cancelled.' : job.status === 'queued' ? 'Vertiku keeps your place safely and starts this book after every earlier book is complete.' : job.destination === 'output' ? 'Vertiku is working directly inside /output without creating a second result copy.' : 'Vertiku is preparing your private browser download.'}</p><progress max="100" value={job.progress} aria-label="Conversion progress" /><strong>{job.progress}%</strong>{job.error && <p className="message error" role="alert">{job.error.message}</p>}<div className="button-row centered">{!['completed', 'failed', 'cancelled'].includes(job.status) && <button className="secondary" onClick={() => void cancel()}>Cancel</button>}{job.downloadReady && <a className="button primary" href={`/api/jobs/${job.id}/download`}>Download M4B</a>}</div></section>;
}

function App() {
  const [boot, setBoot] = useState<'loading' | 'setup' | 'login' | 'ready'>('loading'); const [session, setSession] = useState<Session>();
  const [view, setView] = useState<'convert' | 'jobs' | 'about'>('convert'); const [activeJob, setActiveJob] = useState<Job>(); const [jobs, setJobs] = useState<Job[]>([]); const [menu, setMenu] = useState(false);
  ...
  useEffect(() => { void (async () => { try { const next = await requestJson<Session>('/api/session'); setSession(next); setBoot('ready'); } catch { const status = await requestJson<{ required: boolean }>('/api/setup/status'); setBoot(status.required ? 'setup' : 'login'); } })(); }, []);
  useEffect(() => {
    if (!session || view !== 'jobs') return;
    let disposed = false;
    const refresh = () => void requestJson<{ jobs: Job[] }>('/api/jobs').then((result) => { if (!disposed) setJobs(result.jobs); }).catch(() => undefined);
    refresh(); const events = new EventSource('/api/jobs/events'); events.addEventListener('jobs', (event) => { if (!disposed) setJobs((JSON.parse((event as MessageEvent).data) as { jobs: Job[] }).jobs); });
