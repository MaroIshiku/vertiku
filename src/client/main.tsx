...
type QueueSummary = { remainingJobs: number; queuedJobs: number; estimatedRemainingSeconds: number; estimatedFinishAt?: string; currentJobId?: string; currentJobEstimatedRemainingSeconds?: number; currentJobEstimatedFinishAt?: string; confidence: 'learning' | 'measured' };
...
function QueueForecast({ queue }: { queue?: QueueSummary }) {
  ...
  return <div className="queue-forecast" role="status"><p><strong>Full queue estimate</strong><span>{queue.remainingJobs} audiobook{queue.remainingJobs === 1 ? '' : 's'} remaining</span></p><p>About {formatEstimate(queue.estimatedRemainingSeconds)} · all queued work should finish around {finishTime(queue.estimatedFinishAt)}</p><small>Includes the running audiobook and all {queue.queuedJobs} waiting in the serial queue · {queue.confidence === 'measured' ? 'based on this server' : 'early estimate'}</small></div>;
}
...
function App() {
  ...
  return <div className="app-shell"><header className="topbar"><div className="topbar-inner"><button className="brand" onClick={openConverter}><span className="brand-symbol"><img src="/vertiku-icon.png" alt="" width="42" height="42" /></span><span><strong>Vertiku</strong><small>Everything in the right format.</small></span></button><span className="spacer" /><button className="profile-button" aria-expanded={menu} aria-haspopup="dialog" aria-label="Open profile and settings" onClick={() => setMenu(true)}>{session.user.username.slice(0, 2).toUpperCase()}</button></div></header>
    <main className="main-content">
      ...
      {view === 'jobs' && <section className="workspace-card psu-card"><div className="section-heading"><div><p className="eyebrow">LIVE QUEUE</p><h1>Your conversion jobs</h1><p>Updates arrive automatically. Vertiku processes exactly one audiobook at a time.</p><QueueForecast queue={queue} /></div><div className="button-row">{retryableJobs > 0 && <button className="secondary" onClick={() => void retryFailed()}>Retry {retryableJobs} failed</button>}<button className="primary" onClick={openConverter}>New audiobook</button></div></div>{jobs.length === 0 ? <div className="empty-state"><img src="/vertiku-icon.png" alt="" /><h2>No jobs yet</h2><p>Your audiobook conversions will appear here.</p></div> : <div className="job-groups">{jobGroups.map((group) => <section className="job-group" key={group.id} aria-labelledby={`job-group-${group.id}`}><div className="job-group-heading"><h2 id={`job-group-${group.id}`}>{group.title}</h2><span>{group.jobs.length}</span></div><div className="jobs-list">{group.jobs.map((job) => <button key={job.id} onClick={() => showJob(job)}><span><strong>{job.title}</strong><small>{job.createdAt ? new Date(job.createdAt).toLocaleString() : ''}{job.retryOf ? ' · Retry' : ''}{job.status === 'queued' && job.queuePosition ? ` · Position ${job.queuePosition}` : ''}</small></span><span className="job-list-state"><span className={`status-label ${job.status}`}>{phaseLabel(job)}</span><small>{job.progress}%</small></span></button>)}</div></section>)}</div>}</section>}
      {view === 'about' && <section className="workspace-card psu-card about-card"><img className="app-icon" src="/vertiku-icon.png" alt="" /><p className="eyebrow">ABOUT</p><h1>Vertiku</h1><p>A private, self-hosted conversion workspace. Vertiku creates chaptered M4B audiobooks from uploads or read-only folders below <code>/input</code>.</p><dl><div><dt>Version</dt><dd>0.4.0</dd></div><div><dt>Conversion engine</dt><dd>FFmpeg (runtime detected)</dd></div><div><dt>Privacy</dt><dd>No remote URL imports or outbound media services</dd></div></dl><p className="fine-print">Vertiku does not bypass DRM, includes no yt-dlp integration, and has no dependency on Pulliku.</p></section>}
    </main>
