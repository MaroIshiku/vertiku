...
type QueueSummary = { remainingJobs: number; estimatedRemainingSeconds: number; estimatedFinishAt?: string; confidence: 'learning' | 'measured' };
...
function App() {
  ...
  return <div className="app-shell"><header className="topbar"><div className="topbar-inner"><button className="brand" onClick={openConverter}><span className="brand-symbol"><img src="/vertiku-icon.png" alt="" width="42" height="42" /></span><span><strong>Vertiku</strong><small>Everything in the right format.</small></span></button><span className="spacer" /><button className="profile-button" aria-expanded={menu} aria-haspopup="dialog" aria-label="Open profile and settings" onClick={() => setMenu(true)}>{session.user.username.slice(0, 2).toUpperCase()}</button></div></header>
    <main className="main-content"><nav className="desktop-nav" aria-label="Primary"><button aria-current={view === 'convert' ? 'page' : undefined} onClick={openConverter}>⇄ Convert</button><button aria-current={view === 'jobs' ? 'page' : undefined} onClick={() => setView('jobs')}>▷ Jobs</button><button aria-current={view === 'about' ? 'page' : undefined} onClick={() => setView('about')}>ⓘ About</button></nav>{view === 'convert' && (activeJob ? <JobProgress job={activeJob} session={session} onChange={setActiveJob} /> : <Converter session={session} onJob={showJob} onBatchQueued={showBatch} />)}{view === 'jobs' && <section className="workspace-card psu-card"><div className="section-heading"><div><p className="eyebrow">LIVE QUEUE</p><h1>Your conversion jobs</h1><p>Updates arrive automatically. Vertiku processes exactly one audiobook at a time.</p>{queue && queue.remainingJobs > 0 && <p className="queue-forecast"><strong>{queue.remainingJobs} remaining</strong> · approximately {Math.max(1, Math.round(queue.estimatedRemainingSeconds / 60))} minutes · finish around {queue.estimatedFinishAt ? new Date(queue.estimatedFinishAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'calculating'} <small>({queue.confidence === 'measured' ? 'based on this server' : 'early estimate'})</small></p>}</div><div className="button-row">{retryableJobs > 0 && <button className="secondary" onClick={() => void retryFailed()}>Retry {retryableJobs} failed</button>}<button className="primary" onClick={openConverter}>New audiobook</button></div></div>{jobs.length === 0 ? <div className="empty-state"><img src="/vertiku-icon.png" alt="" /><h2>No jobs yet</h2><p>Your audiobook conversions will appear here.</p></div> : <div className="jobs-list">{jobs.map((job) => <button key={job.id} onClick={() => showJob(job)}><span><strong>{job.title}</strong><small>{job.createdAt ? new Date(job.createdAt).toLocaleString() : ''}{job.retryOf ? ' · Retry' : ''}</small></span><span className="job-list-state"><span className={`status-label ${job.status}`}>{phaseLabel(job)}</span><small>{job.progress}%</small></span></button>)}</div>}</section>}{view === 'about' && <section className="workspace-card psu-card about-card"><img className="app-icon" src="/vertiku-icon.png" alt="" /><p className="eyebrow">ABOUT</p><h1>Vertiku</h1><p>A private, self-hosted conversion workspace. Vertiku creates chaptered M4B audiobooks from uploads or read-only folders below <code>/input</code>.</p><dl><div><dt>Version</dt><dd>0.3.0</dd></div><div><dt>Conversion engine</dt><dd>FFmpeg (runtime detected)</dd></div><div><dt>Privacy</dt><dd>No remote URL imports or outbound media services</dd></div></dl><p className="fine-print">Vertiku does not bypass DRM, includes no yt-dlp integration, and has no dependency on Pulliku.</p></section>}</main>
